...
def generate_road_mask_from_trajectory(
    ...
    valid_mask = (
        (traj_pixels[:, 0] >= 0) & (traj_pixels[:, 0] < bev_height) &
        (traj_pixels[:, 1] >= 0) & (traj_pixels[:, 1] < bev_width)
    )
...
def generate_vehicle_mask_from_agents(
    agents: np.ndarray,
    ...
    for agent, valid in zip(agents, agent_labels):
        ...
        center_pixel = ego_to_bev_coordinates(np.array([[x, y]]), bev_height, bev_width, resolution)[0]
        
        # Skip if outside image
        if (center_pixel[0] < 0 or center_pixel[0] >= bev_height or
            center_pixel[1] < 0 or center_pixel[1] >= bev_width):
            continue
