    ...
    def test_lane_point_format(self):
        ...
        for road_id, road in map_dict.items():
            for lane_id, lane_segments in road.items():
                if (
                    lane_id != "Trigger_Volumes"
                    and isinstance(lane_segments, list)
                    and len(lane_segments) > 0
                ):
        ...
        for road_id, road in map_dict.items():
            if "Trigger_Volumes" in road and len(road["Trigger_Volumes"]) > 0:
                ...
                assert trigger["Type"] in [
                    "TrafficLight",
                    "StopSign",
                ], f"Unknown trigger type: {trigger['Type']}"
...
class TestBEVMapGeneration:
    ...
    def test_generate_bev_from_lanes(self):
        ...
        lanes = {
            "Solid": [np.array([[5, -2], [10, -2], [15, -1.5], [20, -1]])],
            "Broken": [np.array([[5, 2], [10, 2], [15, 1.5], [20, 1]])],
            "Center": [np.array([[5, 0], [10, 0], [15, 0], [20, 0]])],
        }
...
class TestOfflineBEVGeneration:
    ...
    def test_generate_and_save_bev(self):
        ...
        with tempfile.NamedTemporaryFile(suffix=".npz", delete=False) as tmp:
            np.savez_compressed(
                tmp.name, bev_map=bev_data["bev_map"], metadata=json.dumps(bev_data["metadata"])
            )
            tmp_path = tmp.name
